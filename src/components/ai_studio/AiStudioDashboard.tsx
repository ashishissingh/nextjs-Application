...
import type { Prompt, PromptStatus, DataSourceItem, DataSourceType, DataSourceStatus, ToolItem, ToolStatus, ToolCategory, BotStatus } from '../../../types'; 
import ModuleKitScreen from '../shared/ModuleKitScreen'; 
import GenericKitLikeContent from '../shared/GenericKitLikeContent';
// ScreenSearchBarHeader removed from global imports, used by children if needed
import BotManagementScreen from './BotManagementScreen'; 
import AiStudioSectionHeader from './AiStudioSectionHeader'; // Import the new header
...
import { 
  ...
  CubeIcon,
  CommandLineIcon,
  BotIcon as BotSectionIcon, // Renamed to avoid conflict
  ...
  CheckIcon,
  RectangleStackIcon,
  FolderOpenIcon, // For Knowledge Base items
  GlobeAltIcon, // For website type
  DocumentTextIcon // For PDF/Document type
} from '../../../constants';
...
interface HeaderConfig {
  ...
  showSearchBar?: boolean; // New: to control search bar visibility
...
interface GenericKitSectionConfig<T extends DataSourceItem | ToolItem> {
  ...
  featurePoints: string[];
  primaryActionText: string; // Used for "Create New" button when items exist
  ...
  dataItems?: T[];
  dataItemTableHeaders?: Array<{ key: keyof T | 'actions'; label: string; className?: string; render?: (item: T) => React.ReactNode }>;
...
type AiStudioNavConfigEntry =
  ...
  | GenericKitSectionConfig<DataSourceItem> // For Knowledge Base
  | GenericKitSectionConfig<ToolItem>;      // For Tools
...
// --- PROMPT MANAGEMENT SCREEN (Simplified, will be moved to its own file eventually but modified here for now) ---
const samplePrompts: Prompt[] = Array.from({ length: 30 }, (_, i) => ({ 
  id: `p${i + 1}`, 
  name: `Prompt Title ${i + 1} - ${i % 2 === 0 ? 'Customer Service Query Handler' : 'Creative Content Generation Helper'}`, 
  description: `This is detailed description for prompt ${i + 1}. It explains its purpose, how it's used, and potential outcomes. For example, a customer service prompt might handle greetings, while a content prompt might generate blog ideas. This text is intentionally made longer to test truncation or tooltips if needed later.`, 
  version: `v1.${i % 4}.${i % 2}`, 
  lastModified: new Date(Date.now() - i * 1000 * 60 * 60 * 12).toLocaleDateString(), 
  status: i % 3 === 0 ? 'Published' : i % 3 === 1 ? 'Draft' : 'Archived', 
  tags: i % 2 === 0 ? ['customer-service', 'greeting', `release-${i%4}`, 'faq'] : ['content', 'ideation', 'marketing', `beta-v${i%3}`, 'blogging'] 
}));

const PROMPTS_PER_PAGE = 8; // Increased items per page for table view

const getStatusColorClasses = (status: PromptStatus | DataSourceStatus | ToolStatus | BotStatus): string => {
  switch (status) {
    case 'Published':
    case 'Indexed':
    case 'Available':
    ...
    case 'Draft':
    case 'Pending Setup':
      return 'bg-green-100 text-green-700'; // Draft and Pending Setup are often positive/neutral starting points
    case 'Active': // BotStatus
      return 'bg-green-100 text-green-700';
    case 'Archived':
    case 'Pending':
      return 'bg-slate-100 text-slate-600';
    case 'Inactive': // BotStatus
      return 'bg-slate-100 text-slate-600';
    case 'Processing':
    case 'Syncing': // For DataSource
      return 'bg-yellow-100 text-yellow-700';
    case 'Failed':
    case 'Error': // For BotStatus or ToolStatus
    case 'Disconnected': // For ToolStatus
      return 'bg-red-100 text-red-700';
    case 'Training': // For BotStatus
      return 'bg-yellow-100 text-yellow-700';
    case 'Deprecated': // For ToolStatus
...
const PromptListHeader: React.FC = () => (
  <div className="flex items-center px-4 py-3 bg-slate-100/80 backdrop-blur-sm rounded-t-lg shadow-sm text-xs font-semibold text-slate-600 uppercase tracking-wider sticky top-0 z-10">
...
const PromptListItem: React.FC<PromptListItemProps> = ({ prompt }) => {
  ...
  return (
    <li className="flex items-center px-4 py-3 border-b border-slate-200/80 bg-white/70 hover:bg-slate-50/70 transition-colors duration-150 text-sm">
      <div className="flex-auto w-3/12 px-2 truncate">
        <span className="font-medium text-purple-700 hover:text-purple-800" title={prompt.name}>{prompt.name}</span>
      </div>
      <div className="flex-auto w-2/12 px-2 text-center">
        <span className={`px-2.5 py-1 text-xs font-semibold rounded-full ${getStatusColorClasses(prompt.status)}`}>
          {prompt.status}
        </span>
      </div>
      <div className="flex-auto w-1/12 px-2 text-center text-slate-500">{prompt.version}</div>
      <div className="flex-auto w-2/12 px-2 text-center text-slate-500">{prompt.lastModified}</div>
      <div className="flex-auto w-2/12 px-2 text-center">
        {prompt.tags && prompt.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 justify-center">
            {prompt.tags.slice(0, 2).map(tag => (
              <span key={tag} className="px-1.5 py-0.5 text-xs bg-slate-200 text-slate-700 rounded-md truncate" title={tag}>{tag}</span>
            ))}
            {prompt.tags.length > 2 && (
              <span className="px-1.5 py-0.5 text-xs bg-slate-300 text-slate-700 rounded-md" title={prompt.tags.slice(2).join(', ')}>
                +{prompt.tags.length - 2}
              </span>
            )}
          </div>
  ...
  return (
    <> 
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 className="text-2xl sm:text-3xl font-semibold text-slate-800">Your Prompts</h2>
        <button
      ...
      {currentPrompts.length > 0 ? (
        <div className="bg-white/60 backdrop-blur-sm shadow-md rounded-lg overflow-hidden">
         ...
         <div className="text-center py-10 text-slate-500 bg-white/70 backdrop-blur-md p-6 rounded-xl shadow-lg flex flex-col items-center justify-center min-h-[200px]">
           <PromptSectionIcon className="w-16 h-16 text-slate-400 mb-4" />
           {searchTerm ? 'No prompts found matching your search.' : 'No prompts available. Click "Create New Prompt" to get started.'}
         </div>
      )}
...
// Sample Data Sources for Knowledge Base
const sampleDataSources: DataSourceItem[] = Array.from({ length: 15 }, (_, i) => ({
  id: `ds${i + 1}`,
  name: i % 3 === 0 ? `Product Manual v${i+1}.pdf` : i % 3 === 1 ? `Company Wiki - Section ${String.fromCharCode(65 + i)}` : `Support Tickets Q${(i%4)+1} Export.txt`,
  type: i % 3 === 0 ? 'PDF' : i % 3 === 1 ? 'Website' : 'Text File',
  status: ['Indexed', 'Processing', 'Failed', 'Pending', 'Available', 'Syncing'][i % 6] as DataSourceStatus,
...
const knowledgeBaseTableHeaders: Array<{ key: keyof DataSourceItem | 'actions'; label: string; className?: string; render?: (item: DataSourceItem) => React.ReactNode }> = [
  ...
  { key: 'size', label: 'Size/Items', className: 'w-1/12 text-center', render: (item) => item.size || (item.itemCount ? `${item.itemCount} items` : '-') },
  { key: 'actions', label: 'Actions', className: 'w-1/12 text-right' },
];

// Sample Tools for Tools Section
const sampleTools: ToolItem[] = Array.from({ length: 12 }, (_, i) => ({
    id: `tool${i + 1}`,
    name: `API Connector ${String.fromCharCode(65 + i)}`,
    description: `Connects to external service ${String.fromCharCode(65 + i)} for enhanced data processing and actions. Version ${i%3 +1}.0.`,
    category: ['API', 'Service', 'Plugin', 'Custom', 'Data Store'][i % 5] as ToolCategory,
    status: ['Connected', 'Disconnected', 'Error', 'Pending Setup', 'Deprecated'][i % 5] as ToolStatus,
    version: `v${i%3 +1}.${i%2}.${i%4}`,
    lastConnected: new Date(Date.now() - i * 1000 * 60 * 60 * 48).toLocaleString(),
    tags: [`service-${String.fromCharCode(65 + i).toLowerCase()}`, ['beta', 'stable', 'alpha'][i%3]],
    owner: i % 2 === 0 ? 'System' : 'User: jane.doe',
}));

const toolTableHeaders: Array<{ key: keyof ToolItem | 'actions'; label: string; className?: string; render?: (item: ToolItem) => React.ReactNode }> = [
...
const defaultHeaderBg = 'bg-gradient-to-r from-slate-800 via-indigo-900 to-purple-800'; 
const moduleKitsHeaderBg = 'bg-gradient-to-br from-slate-900 via-purple-900 to-indigo-900';
...
const navContentConfig: Record<AiStudioNavId, AiStudioNavConfigEntry> = {
  ...
  prompts: { 
    ...
    headerConfig: {
      ...
      decorativeIcon: <PromptSectionIcon />,
      backgroundColorClasses: defaultHeaderBg,
      showSearchBar: true,
  ...
  knowledgeBase: { 
    ...
    setupDescription: "Connect documents, websites, and databases to provide relevant context for your AI models and bots.",
    featurePoints: [
    ...
    headerConfig: {
      ...
      decorativeIcon: <KnowledgeBaseIcon />,
      backgroundColorClasses: defaultHeaderBg,
      showSearchBar: true,
  ...
  tools: { 
    ...
    setupDescription: "Extend AI Studio capabilities by connecting external services, APIs, and custom tools.",
    featurePoints: [
    ...
    headerConfig: {
      ...
      decorativeIcon: <ToolsSectionIcon />,
      backgroundColorClasses: defaultHeaderBg,
      showSearchBar: true,
  ...
  moduleKits: { 
    ...
    headerConfig: {
      title: "Module Kits", 
      decorativeIcon: <RectangleStackIcon />, 
    ...
    {title && (
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          {icon && React.cloneElement(icon, { className: "w-6 h-6 text-purple-600 mr-3" })}
...
const ListItemAction: React.FC<{icon: React.ReactElement<{ className?: string }>, text: string, subtext?: string, actionIcon?: React.ReactElement<{ className?: string }>, tag?: string }> = ({ icon, text, subtext, actionIcon, tag }) => (
  <a href="#" className="flex items-center justify-between p-3 -mx-3 rounded-lg hover:bg-slate-100 transition-colors duration-150 group">
    <div className="flex items-center">
      {React.cloneElement(icon, { className: "w-6 h-6 text-slate-500 mr-4 group-hover:text-purple-600" })}
      <div>
        <span className="text-slate-700 font-medium group-hover:text-purple-700">{text}</span>
        {subtext && <p className="text-sm text-slate-500">{subtext}</p>}
      </div>
    </div>
    <div className="flex items-center">
      {tag && <span className={`mr-2 px-2 py-0.5 text-xs font-semibold rounded-full ${tag === 'New' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600'}`}>{tag}</span>}
      {actionIcon && React.cloneElement(actionIcon, { className: "w-5 h-5 text-slate-400 group-hover:text-purple-600"})}
    </div>
  </a>
);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <WidgetCard className="md:col-span-2" icon={<SparklesIcon />} title={`Good morning, ${userName}!`}>
          <p className="text-slate-600">
            Here's a quick overview of your workspace. You are doing great!
            Welcome back to AI Studio. You can customize this dashboard to fit your needs. Use the "Customize" button to add or remove widgets.
          </p>
        </WidgetCard>

        <WidgetCard title="Quick Actions" icon={<CommandLineIcon />}>
          <ul className="space-y-1">
            <li><ListItemAction icon={<DocumentPlusIcon />} text="Request Module Access" actionIcon={<ChevronRightIcon />} /></li>
            <li><ListItemAction icon={<UserCircleIcon />} text="View Your Profile" actionIcon={<ChevronRightIcon />} /></li>
            <li><ListItemAction icon={<ChatBubbleLeftRightIcon />} text="Contact Support" actionIcon={<ChevronRightIcon />} /></li>
          </ul>
        </WidgetCard>

        <WidgetCard title="My Modules" icon={<CubeIcon />}>
        <p className="text-sm text-slate-500 mb-3">Modules you have direct access to.</p>
          <ul className="space-y-1 max-h-48 overflow-y-auto">
            <li><ListItemAction icon={<CommandLineIcon />} text="Finance Tools (Legacy)" actionIcon={<ChevronRightIcon />} /></li>
            <li><ListItemAction icon={<BuildingStorefrontIcon />} text="Inventory Control (Legacy)" actionIcon={<ChevronRightIcon />} /></li>
            <li><ListItemAction icon={<ChatBubbleLeftRightIcon />} text="CRM Platform (Legacy)" actionIcon={<ChevronRightIcon />} /></li>
            <li><ListItemAction icon={<DocumentPlusIcon />} text="Request Access" actionIcon={<ChevronRightIcon />} /></li>
          </ul>
        </WidgetCard>
        ...
        <WidgetCard 
          ...
          titleAction={<a href="#" className="text-sm font-medium text-purple-600 hover:text-purple-800">View all <ChevronRightIcon className="inline w-4 h-4" /></a>}
        >
          <p className="text-sm text-slate-500 mb-3">You have 4 unread notifications.</p>
          <ul className="space-y-2">
            <li>
              <ListItemAction 
                ...
                tag="New"
                actionIcon={<ChevronRightIcon className="text-green-500" />}
              />
            </li>
            <li>
              <ListItemAction 
        ...
        <WidgetCard title="Bot Activity" icon={<ArrowTrendingUpIcon />}>
          <p className="text-sm text-slate-500 mb-2">Monthly bot interactions and active counts.</p>
          <div className="bg-slate-100 p-4 rounded-lg min-h-[150px] flex items-center justify-center">
            <p className="text-slate-400 italic">Chart placeholder</p>
          </div>
        </WidgetCard>
      </div>
    </>
...
const AiStudioDashboard: React.FC<AiStudioDashboardProps> = ({ activeNav }) => {
  ...
  if (!navItemConfig) {
    contentToRender = <p className="text-xl text-red-500 italic">Error: Invalid AI Studio navigation section selected.</p>;
    currentHeaderConfig = { title: "Error", backgroundColorClasses: defaultHeaderBg, showSearchBar: false }; 
  } else {
    ...
    switch (navItemConfig.navType) {
      ...
      case 'moduleKitsAiStudioConfig':
        contentToRender = <navItemConfig.Component {...navItemConfig.propsForComponent} />;
        break;
        ...
        if (navItemConfig.itemTypeForIcons === 'datasource') {
          ...
          contentToRender = (
              <GenericKitLikeContent<DataSourceItem>
                ...
                onPrimaryActionClick={() => console.log(`Primary action for ${config.headerConfig.title}`)}
                ...
                emptyStateIcon={config.headerConfig.decorativeIcon || <KnowledgeBaseIcon className="w-16 h-16 text-slate-400 mb-4" />}
          ...
          contentToRender = (
              <GenericKitLikeContent<ToolItem>
                ...
                emptyStateText={config.emptyStateText}
                emptyStateSetupText={config.emptyStateSetupText}
                emptyStateIcon={config.headerConfig.decorativeIcon || <ToolsSectionIcon className="w-16 h-16 text-slate-400 mb-4" />}
                dataItems={config.dataItems}
                dataItemTableHeaders={config.dataItemTableHeaders}
                itemTypeForIcons={config.itemTypeForIcons}
                onEditItem={config.onEditItem}
          ...
          contentToRender = <p className="text-xl text-red-500 italic">Error: Invalid itemTypeForIcons in configuration.</p>;
          currentHeaderConfig = { title: "Configuration Error", backgroundColorClasses: defaultHeaderBg, showSearchBar: false };
        }
        break;
      default:
        const exhaustiveCheck: never = navItemConfig;
        console.error("AiStudioDashboard: Unhandled navType or configuration structure.", activeNav, exhaustiveCheck);
        contentToRender = <p className="text-xl text-red-500 italic">Configuration error for this section.</p>;
        currentHeaderConfig = { title: "Error", backgroundColorClasses: defaultHeaderBg, showSearchBar: false }; 
        break;
    }
  }
  ...
  return (
    <div className="flex-1 flex flex-col bg-slate-50 overflow-hidden h-full">
      ...
      <footer className="shrink-0 p-4 border-t border-slate-200 bg-slate-50/80 backdrop-blur-sm text-center text-sm text-slate-600">
...
// Common function from AiStudioDashboard, also useful here.
// Duplicated to avoid import complexity, consider moving to a shared utils file if more widely needed.
// This is now defined in GenericKitLikeContent.tsx and AiStudioDashboard.tsx (for Prompts), consider consolidating.
const getDataSourceStatusColorClasses = (status: DataSourceStatus): string => {
  switch (status) {
    case 'Indexed':
    case 'Available':
      return 'bg-blue-100 text-blue-700';
