...
import { getAuthorizeEndpoint } from '@/lib/authorizedEndpoint';
import { apiConfig, CONFIG } from '@/lib/config';
import { AuthenticationDetails, CognitoUser, CognitoUserPool } from 'amazon-cognito-identity-js';
import { processTokens } from '@/lib/loading/auth-processing';
import { sendOrgOwnerConfirmation } from '@/lib/sendOrgOwnerConfirmation';
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    user.authenticateUser(authDetails, {
      onSuccess: async (data) => {
        ...
        processTokens(IdToken, AccessToken, RefreshToken);

        const isOrgOwner = localStorage.getItem('orgflag');
        const orgid = localStorage.getItem('orgid');
        const email = values.email;
        const apiUrl = CONFIG.apiUrl;

        if (isOrgOwner === 'true' && orgid && email && apiUrl) {
          console.log("Sending confirmation for org owner...");
          await sendOrgOwnerConfirmation(email, orgid, apiUrl);
        }
      ...
      onFailure: (err) => {
        ...
        setLoadingApp(false);
        const wup = document.getElementById("wrondUNPW");
        if (wup) wup.style.display = "block";
      },
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <Button variant="outline" type="button" aria-label="Continue with Microsoft" onClick={() => { window.location.href = getAuthorizeEndpoint(); }}>
